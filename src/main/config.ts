...
export function decryptPassword(config: StoredDashboardConfig): string {
  if (
    config.kindlePasswordEncoding === "safeStorage" &&
    config.kindlePasswordEncrypted
  ) {
    return safeStorage.decryptString(
      Buffer.from(config.kindlePasswordEncrypted, "base64"),
    );
  }
  return config.kindlePasswordPlain ?? "";
}

function encryptedPasswordFields(
  password: string,
): Partial<StoredDashboardConfig> {
  if (safeStorage.isEncryptionAvailable()) {
    return {
      kindlePasswordEncrypted: safeStorage
...
export async function loadConfig(): Promise<StoredDashboardConfig> {
  ...
  try {
    ...
    ) as Partial<StoredDashboardConfig>;
    dashboardConfig = {
      ...defaults,
      imageUrl:
        typeof raw.imageUrl === "string" ? raw.imageUrl : defaults.imageUrl,
      language: normalizeLanguagePreference(raw.language),
      kindleFullRefreshEvery: positiveInt(
        String(raw.kindleFullRefreshEvery ?? ""),
        defaults.kindleFullRefreshEvery,
      ),
      kindleIp:
        typeof raw.kindleIp === "string" ? raw.kindleIp : defaults.kindleIp,
      kindlePasswordEncoding: raw.kindlePasswordEncoding,
      kindlePasswordEncrypted:
        typeof raw.kindlePasswordEncrypted === "string"
          ? raw.kindlePasswordEncrypted
          : undefined,
      kindlePasswordPlain:
        typeof raw.kindlePasswordPlain === "string"
          ? raw.kindlePasswordPlain
          : undefined,
      kindlePort: positiveInt(
        String(raw.kindlePort ?? ""),
        defaults.kindlePort,
      ),
      kindleRefreshInterval: positiveInt(
        String(raw.kindleRefreshInterval ?? ""),
        defaults.kindleRefreshInterval,
      ),
      kindleUser:
        typeof raw.kindleUser === "string"
          ? raw.kindleUser
          : defaults.kindleUser,
      kindleWifiRetryEvery: positiveInt(
        String(raw.kindleWifiRetryEvery ?? ""),
        defaults.kindleWifiRetryEvery,
      ),
      setupComplete:
        raw.setupComplete === true && typeof raw.imageUrl === "string",
    };
  } catch {
...
function requiredString(
  ...
  if (typeof value !== "string")
    throw new Error(text("configText", { field: key }));
  const trimmed = value.trim();
  if (!trimmed) throw new Error(text("configRequired", { field: key }));
  if (trimmed.length > maxLength)
    throw new Error(text("configTooLong", { field: key }));
  return trimmed;
}
...
function numberField(
  ...
  const value =
    typeof input[key] === "number"
      ? (input[key] as number)
      : Number.parseInt(String(input[key] ?? ""), 10);
...
export async function saveConfig(raw: unknown): Promise<DashboardConfig> {
  ...
  const password =
    typeof input.kindlePassword === "string" ? input.kindlePassword : "";
  const next: StoredDashboardConfig = {
    ...previous,
    imageUrl: normalizedImageUrl(requiredString(input, "imageUrl", 500)),
    kindleFullRefreshEvery: numberField(
      input,
      "kindleFullRefreshEvery",
      previous.kindleFullRefreshEvery,
      1000,
    ),
    kindleIp: requiredString(input, "kindleIp", 255),
    kindlePort: numberField(input, "kindlePort", previous.kindlePort),
    kindleRefreshInterval: numberField(
      input,
      "kindleRefreshInterval",
      previous.kindleRefreshInterval,
      86400,
    ),
    kindleUser: requiredString(input, "kindleUser", 64),
    kindleWifiRetryEvery: numberField(
      input,
      "kindleWifiRetryEvery",
      previous.kindleWifiRetryEvery,
      1000,
    ),
  };
